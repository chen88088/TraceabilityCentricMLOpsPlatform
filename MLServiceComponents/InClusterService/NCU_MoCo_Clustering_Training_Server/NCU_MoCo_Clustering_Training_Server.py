...
async def health_check():
    return JSONResponse(content={"status": "NCU_MoCo_Clustering_Training_MLServingPod is healthy"})
...
async def setup_folders_for_training(request: DagRequest):
    ...
    # 組合路徑：/mnt/storage/{dag_id}_{execution_id}
    dag_root_folder_path = os.path.join(STORAGE_PATH, f"{dag_id}_{execution_id}")
    repo_inference_path = os.path.join(dag_root_folder_path, "NCU_MoCo_Clustering_Training")
...
async def download_preprocessing_result(request: DagRequest):
    ...
    try:
        ...
        # target
        repo_inference_path = os.path.join(dag_root_folder_path, "NCU_MoCo_Clustering_Training")
        target_result_path = Path(repo_inference_path) / "MoCo_training" /"data"
...
# [Clustering/ExecuteClusteringTrainingScripts]
...
async def execute_inference_scripts(request: DagRequest):
    ...
    try:
        ...
        # **等待 Job 完成**
        job_success = wait_for_job_completion(batch_v1, job_name, "ml-serving",logger)
        ...
        record_clustering_training_result_to_mlflow(request, output_dir, logger)
...
def wait_for_job_completion(batch_v1, job_name, namespace, logger, timeout=108000):
    """
    監聽 K8s Job 狀態，等待 Job 執行完成，成功回 True，失敗/timeout 回 False
    """
    import time
        ...
        if job_status.status.succeeded == 1:
            ...
            if pod_list:
                ...
                try:
                    ...
                    logger.info(f"Logs for {job_name}:\n{logs}")
                except Exception as e:
                    logger.warning(f"Failed to read logs for {pod_name}: {e}")
    ...
    logger.error(f"Job {job_name} timeout after {timeout} seconds.")
    return False
...
def record_clustering_training_result_to_mlflow(request: DagRequest, output_dir: str, logger):
    """ 記錄執行結果到 MLflow """
    ...
    # 找到history 資料夾
    history_csv_path = os.path.join(run_folder, "history", "epoch.csv")

    # 找到model 資料夾
    model_dir = os.path.join(run_folder, "model")
    model_subdirs = [d for d in os.listdir(model_dir) if os.path.isdir(os.path.join(model_dir, d))]

    if model_subdirs:
        model_path = os.path.join(model_dir, model_subdirs[0])  # 取第一個子資料夾
        logger.info(f"找到模型資料夾：{model_path}")
    else:
        model_path = None
        logger.error("警告：未找到 model 內的資料夾！")
    ...
    with mlflow.start_run(run_name=dag_instance_unique_id):
        ...
        for index, row in df.iterrows():
            mlflow.log_metric("acc1", row["acc1"], step=int(row["epoch"]))
            mlflow.log_metric("acc5", row["acc5"], step=int(row["epoch"]))
            mlflow.log_metric("loss", row["loss"], step=int(row["epoch"]))
            mlflow.log_metric("loss_moco", row["loss_moco"], step=int(row["epoch"]))
            mlflow.log_metric("weight_decay", row["weight_decay"], step=int(row["epoch"]))

        # 上傳模型
        mlflow.log_artifacts(model_path, artifact_path="moco_model")
    
    logger.info("成功記錄到 MLflow！")
