...
def train_moco(args, logger, initial_epoch, strategy, num_workers):
    ...
    if os.path.isdir(f'{args.result_path}/{args.task}/{args.stamp}'):
        save_path = os.path.join(f'{args.result_path}/{args.task}/{args.stamp}/model', f'moco_model_{args.batch_size}_{args.epochs}_{args.lr}')
    _ = model.encoder_q(tf.keras.Input(shape=(None, None, 3), dtype=tf.float32))  # Build the encoder_q model
    tf.saved_model.save(model.encoder_q, save_path)
...
def train_lincls(args, logger, initial_epoch, strategy, num_workers):
    ...
    if os.path.isdir(f'{args.result_path}/{args.task}/{args.stamp}'):
        save_path = os.path.join(f'{args.result_path}/{args.task}/{args.stamp}/model', f'lincls_model_{args.batch_size}_{args.epochs}_{args.lr}')
    _ = model(tf.keras.Input(shape=(None, None, 3), dtype=tf.float32))  # Pass a dummy batch through the model to build it
