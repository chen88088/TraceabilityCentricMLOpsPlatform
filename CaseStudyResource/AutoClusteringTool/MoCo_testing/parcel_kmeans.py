...
import os
from config import folder_path, clusters_amount, subfolder_path
...
def do_Kmeans_clustering(image_list, clusters_amount):
    ...
    return flatten_image_list, predictions
...
def do_tsne(data, clusters, title):
    ...
    scatter = ax.scatter(outputs_tsne[:, 0], outputs_tsne[:, 1], outputs_tsne[:, 2], 
                         c=clusters, cmap='viridis', s=point_size)
...
flatten_combined_list, combined_predictions = do_Kmeans_clustering(image_list=combined_list,
                                        clusters_amount=clusters_amount)  # 分群結果 屬於哪個cluster

# 保存分群後的影像
save_clustered_images(combined_names, combined_list, combined_predictions, subfolder_path+'_kmeans')
# 可視化ground truth
do_tsne(flatten_combined_list, combined_GT_list, "t-SNE Visualization of Ground Truth")
# 可視化K-means聚類结果
do_tsne(flatten_combined_list, combined_predictions, "t-SNE Visualization of cluster result")
...
pivot_table.to_csv(os.path.join(subfolder_path+'_kmeans', 'cluster_label_distribution.csv'), encoding="utf-8-sig")
...
# flatten_rice_list, rice_predictions = do_Kmeans_clustering(image_list=rice_list,
#                                         clusters_amount=clusters_amount)  # 分群結果 屬於哪個cluster

# # 可視化 K-means 聚類結果
# do_tsne(flatten_rice_list, rice_predictions, "t-SNE Visualization of Kmeans")

# print("end Rice Clustering")

# print("start Non Rice Clustering")

# flatten_non_rice_list, non_rice_predictions = do_Kmeans_clustering(image_list=non_rice_list,
#                                         clusters_amount=clusters_amount)  # 分群結果 屬於哪個cluster
# # 可視化 K-means 聚類結果
# do_tsne(flatten_non_rice_list, non_rice_predictions, "t-SNE Visualization of Kmeans")
